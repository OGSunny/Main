...
if not isfolder("UNordinary Hub") then
    makefolder("UNordinary Hub")
end
...
function Library:clear()
	for _, object in CoreGui:GetChildren() do
		if object.Name ~= "UNordinary Hub" then
			continue
		end
		object:Destroy()
	end
...
function Library:save_flags()
    ...
    local success, result = pcall(function()
        ...
        writefile(`UNordinary Hub/{game.GameId}.lua`, flags)
    end)
end

function Library:load_flags()
    local success, result = pcall(function()
        if not isfile(`UNordinary Hub/{game.GameId}.lua`) then
            Library.save_flags()
            return
        end

        local flags = readfile(`UNordinary Hub/{game.GameId}.lua`)
        if not flags then
            Library.save_flags()
            return
        end
